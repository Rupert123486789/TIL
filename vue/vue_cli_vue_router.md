...
* **Module 의존성 문제**
  ...
  * 즉, Webpack은 이 모듈 간의 **✨의존성 문제를 해결**하기 위해 등장
...
* **node_module의 의존성 깊이**
  * ![image-20220514105832616](vue_cli_vue_router.assets/image-20220514105832616.png)

<br>

* **Webpack**
  * ![image-20220514105848047](vue_cli_vue_router.assets/image-20220514105848047.png)
...
### 4. Pass Props & Emit Events(component들의 의사소통 방식)

<br>

* **컴포넌트 작성**
  * Vue app은 자연스럽게 중처된 컴포넌트 트리로 구성됨
  * 컴포넌트 간 부모-자식 관계가 구성되며 이들 사이에 필연적으로 의사 소통이 필요함
  * ![image-20220514110622057](vue_cli_vue_router.assets/image-20220514110622057.png)
...
* **Pass Props & Emit Events**
  * ![image-20220514111055027](vue_cli_vue_router.assets/image-20220514111055027.png)
...
* **💥컴포넌트의 'data'는 반드시 함수여야 함**
  * 기본적으로 각 인스턴스는 모두 같은 data 객체를 공유하므로 새로운 data 객체를 반환(return)하여야 함
    * ![image-20220514111615178](vue_cli_vue_router.assets/image-20220514111615178.png)
...
* **Props 시 자주하는 실수**
  ...
  * 실제 JavaScript 숫자를 전달하려면 값이 JavaScript 표현식으로 평가되도록 v-bind를 사용해야함
    * ![image-20220514111707594](vue_cli_vue_router.assets/image-20220514111707594.png)
...
* **event 이름 컨벤션**
  ...
  * HTML의 대소문자 구분을 위해 DOM 템플릿의 v-on 이벤트 리스너는 항상 자동으로 소문자 변환되기 때문에 v-on:myEvent는 자동을 v-on:myevent로 변환(✨camelCase 인지 못함)
  * 이러한 이유로 이벤트 이름에는 **💥항상 kebab-case를 사용하는 것을 권장**
    * ![image-20220514112511054](vue_cli_vue_router.assets/image-20220514112511054.png)
...
* **History mode**
  ...
  * 브라우저의 히스토리는 남기지만 실제 페이즈는 이동하지 않는 기능을 지원
  * 즉, 페이지를 다시 로드하지 않고 URL을 탐색할 수 있음
    * SPA의 단점 중 하나인 "URL이 변경되지 않는다."를 해결
  * ![image-20220514113601978](vue_cli_vue_router.assets/image-20220514113601978.png)
...
* **2. 프로그래밍 방식 네비게이션**
  * `<router-link>`를 사용하여 선언적 탐색을 위한 a 태그를 만드는 것 외에도, router의 인스턴스 메서드를 사용하여 프로그래밍 방식으로 같은 작업을 수행 할 수 있음
    * ![image-20220514113936499](vue_cli_vue_router.assets/image-20220514113936499.png)
  * Vue 인스턴스 내부에서 라우터 인스턴스에 ✨**$route**r로 접근할 수 있음
  ...
  * 작성할 수 있는 인자 예시
    * ![image-20220514114152573](vue_cli_vue_router.assets/image-20220514114152573.png)
...
* **3. Dynamic Router Matching**
  ...
  * 예를 들어 모든 User에 대해 동일한 레이아웃을 가지지만, 다른 User ID로 렌더링 되어야하는 User 컴포넌트 예시
    * ![image-20220514114313906](vue_cli_vue_router.assets/image-20220514114313906.png)
  * 동적 인자는 :(콜론)으로 시작
  * 컴포넌트에서 **✨this.$route.params**로 사용가능
    * ![image-20220514114352070](vue_cli_vue_router.assets/image-20220514114352070.png)
  * LottoView.vue 예시
  * TheLotto.vue 예시
