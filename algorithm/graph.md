...
* **MST 표현(1)**
  * ![image-20220409171143819](graph.assets/image-20220409171143819.png)
  * 연결된 비용(가중치)를 적음

<br>

* **MST 표현(2)**
  * ![image-20220409171224169](graph.assets/image-20220409171224169.png)

<br>

* **Prim 알고리즘**
  * MST를 찾는 알고리즘 2가지 : Prim, Kruskal
  * 하나의 정점에서 연결된 간선들 중에 하나씩 선택하면서 MST를 만들어 가는 방식
    1. 임의의 정점을 하나 선택해서 시작
    2. 선택한 정점과 인접하는 정점들 중의 최소 비용의 간선이 존재하는 정점을 선택
    3. 모든 정점이 선택될 때 가지 1, 2 과정을 반복
  * 서로소인 2개의 집합(2 disjoint-sets) 정보를 유지
    * 트리 정점들(tree vertices) - MST를 만들기 위해 선택된 정점들
    * 비트리 정점들(nontree vertices) - 선택 되지 않은 정점들
  * 알고리즘 적용 예
    * 사이클이 아니면서 MST에서 방금한 것뿐만 아니라 전체에서 연결된 것중 최소를 선택
    * 가장 작은 값이 2개일 땐 아무거나 선택해도 상관 없음
    * ![image-20220409171653567](graph.assets/image-20220409171653567.png)
    * ![image-20220409171754010](graph.assets/image-20220409171754010.png)
  * 알고리즘
    * ![image-20220409171946806](graph.assets/image-20220409171946806.png)
    * 무한 : 가중치 합으로 나오지 않을 큰 값
    * key : 어떤 정점이 MST에 속하는데 필요한 비용
    * w(u, v) : weight
    * 자식 초기값은 0
    * 부모를 넣어서 사이클 유무를 확인
  * 교수님 코드
    * 인접 배열 : 시작접, 끝점, 거리
    * 최소 이동거리만 담아야 하므로 INF로 넣고 갱신
    * 6-5 : 정점의 갯수만큼만 돌면됨 / 어디로 연결되는 간선이 가장 작은지
    * 6-6 : 갱신
    * 6-7 : 거리가 존재하고(본인 선택 X) / 그 정점이 아직 MST로 등록되어 있지 않으면 갱신
...
* **KRUSKAL 알고리즘**
  ...
  * find_set : 같은 부모를 가르키면 사이클
  * union / find_set : 부모가 같지 않으면 정점을 붙임
  * 알고리즘 적용 예
    * ![image-20220409172254937](graph.assets/image-20220409172254937.png)
    * ![image-20220409172340315](graph.assets/image-20220409172340315.png)
  * 알고리즘
    * ![image-20220409172401565](graph.assets/image-20220409172401565.png)
  * 교수님 코드
    * 거리를 오름차순 / finde_set으로 서로 다른 부모인지 확인 => union으로 부모 갱신
    * 7-12 : union을 따로 안 만듬

<br>

* ✨효율
  * 간선이 적당히 있으면(일반적으로는) : Kruskal
  * 빽빽한 완전 그래프이면 : Prim
...
* **Dijkstra 알고리즘**
  ...
  * 탐욕 기법을 사용한 알고리즘으로 MST의 프림 알고리즘과 유사함(Prim은 거리를 따로 저장 / Dijkstra는 누적)
  * 알고리즘 
    * ![image-20220409172903891](graph.assets/image-20220409172903891.png)
  * 알고리즘 적용 예
    * 다익스트라 : 출발점을 정해주면 나머지 모든 정점에 도달하는 최소비용이 다 구해짐
    * ![image-20220409173021794](graph.assets/image-20220409173021794.png)
    * ![image-20220409173047234](graph.assets/image-20220409173047234.png)
    * ![image-20220409173156645](graph.assets/image-20220409173156645.png)
      * 모든 정점의 최소비용이 결정될때까지 반복
      * 선택된 정점의 집합과 주어진 정점의 집합이 일치할 때까지
    * ![image-20220409173332999](graph.assets/image-20220409173332999.png)
    * ![image-20220409173426893](graph.assets/image-20220409173426893.png)
    * ![image-20220409173419373](graph.assets/image-20220409173419373.png)
    * ![image-20220409173548675](graph.assets/image-20220409173548675.png)
    * ![image-20220409173625421](graph.assets/image-20220409173625421.png)
    * ![image-20220409173659918](graph.assets/image-20220409173659918.png)
      * f로 가는 최소 비용을 구하면 b, c, d, e로 가는 최소 비용도 전부 구해짐
      * 최소 왕복 비용을 계산하는법
        * ✨출발 도착 다익스트라와 출발 도착을 뒤집어서한 다익스트라 2개를 쓰면됨
        * 모든 경우를 전부 다익스트라로 돌리면 오래 걸림(a는 b,c,d,e,f로 / b는 a,c,d,e,f 로/ ...)
  * 교수님 코드
    * 12-2,3 : adj를 리스트로 묶어도 되지만 3차원 배열이라 보기 힘듬 / 그래서 튜플로 함 / immutable한 데이터이므로 튜플로 해도 상관 없음
    * 12-4 :  visited - 정점을 지나쳤는지 확인
